...
LEGENDARY_GATE = int(os.getenv("LEGENDARY_GATE", "97"))  # Raised to 97% for elite performance
GLOBAL_THRESHOLD = LEGENDARY_GATE
ASSET_THRESHOLD_OVERRIDE: Dict[str, float] = {}

COOLDOWN_MINUTES = int(os.getenv("COOLDOWN_MINUTES", "15"))
SCAN_INTERVAL_SEC = int(os.getenv("SCAN_INTERVAL_SEC", "30"))  # Faster scanning
...
ASSET_CONFIG = {
    "EURUSD=X": {
        "min_data_points": 50,
        "volatility_threshold": 0.35,  # Tightened
        "vwap_threshold": 0.008,
        "volume_profile_sensitivity": 0.004,
        "trend_strength_multiplier": 1.2,
        "momentum_volume_multiplier": 1.8,
        "min_volume_ratio": 0.7,  # NEW: Minimum volume vs average
        "max_spread_pct": 0.0005,  # NEW: Max spread tolerance
        "choppy_threshold": 0.3  # NEW: ATR/Price ratio for choppiness
    },
    "GBPUSD=X": {
        "min_data_points": 50,
...
REJECTED_SIGNALS: deque = deque(maxlen=500)  # Track why signals were rejected
...
class EconomicCalendar:
    ...
    # High-impact news times (UTC) - customize based on your needs
    NEWS_BLACKOUT_WINDOWS = [
        # NFP (First Friday, 12:30-14:00 UTC)
        # FOMC (Scheduled, 18:00-20:00 UTC)
        # ECB (Scheduled, 11:45-13:30 UTC)
    ]
    ...
    def is_news_time() -> Tuple[bool, str]:
        ...
        # General high-impact news windows
        # US Session Open: 13:30-14:30 UTC (avoid first hour)
        if dt_time(13, 30) <= current_time <= dt_time(14, 30):
            return True, "US Session Open"
        
        # London Session Open: 07:00-08:00 UTC
        ...
        # Asian Session (thin liquidity): 22:00-02:00 UTC
...
class MarketConditionFilter:
    ...
    def is_choppy_market(df: pd.DataFrame, asset_config: Dict) -> Tuple[bool, float]:
        ...
        try:
            ...
            # Choppy if: low ATR + tight range + no clear direction
    ...
    def has_sufficient_liquidity(df: pd.DataFrame, asset_config: Dict) -> Tuple[bool, float]:
        ...
        try:
            ...
            # Also check recent volume trend
            ...
            return True, 1.0  # Default to pass if check fails
    ...
    def check_volatility_spike(df: pd.DataFrame) -> Tuple[bool, float]:
        ...
        try:
            ...
            # Spike if current vol > 2.5x average
...
class RealDataProvider:
    ...
    def fetch_ohlcv(self, symbol: str, timeframe: str = "1d", limit: int = 300) -> Optional[pd.DataFrame]:
        ...
        interval_map = {
            "1d": "1d",
            "4h": "1h",  # yfinance doesn't have 4h, use 1h and resample later
            "1h": "1h",
            "15m": "15m",
            "5m": "5m"
        }
...
def main_scan_loop():
    ...
    # Test data fetching to ensure it works
    ...
    if test_df is not None and not test_df.empty:
        ...
        logger.warning(f"⚠️ Could not fetch test data for {test_asset}")
    ...
    while True:
        try:
            ...
            for asset in ASSETS:
                ...
                for tf in ALL_TIMEFRAMES:
                    try:
                        ...
                        if safe_df_check(df, asset_config.get("min_data_points", 50)):
                            tf_data[tf] = df
                            logger.debug(f"Fetched {asset} {tf}: {len(df)} bars")
                        else:
                            logger.debug(f"Insufficient data for {asset} {tf}")
                    except Exception as e:
                        logger.debug(f"Failed to fetch {asset} {tf}: {e}")

                if len(tf_data) < 3:
                    logger.debug(f"Insufficient data for {asset}, only {len(tf_data)} timeframes")
                    continue

                # Get primary timeframe for condition checks
                primary_df = tf_data.get("1h") or tf_data.get("4h") or tf_data.get("1d")
                if primary_df is None:
                    logger.debug(f"No primary timeframe data for {asset}")
                    continue
                ...
                logger.debug(f"{asset}: Using {engine['name']} (Mode: {EngineState.current_mode})")
                ...
                if result['signal'] == "NEUTRAL":
                    logger.debug(f"{asset}: No signal from analysis")
                    continue
                ...
                if (result['confidence'] >= threshold and
                    ...
                    for expiry in EXPIRIES:
                        ...
                        logger.info(f"🔥 LEGENDARY SIGNAL: {asset} {result['signal']} @ {result['confidence']:.1f}% Expiry: {expiry}m [{engine['name']}]")
                        time.sleep(0.5)  # Small delay between expiry alerts
                else:
                    logger.debug(f"{asset}: Signal below threshold ({result['confidence']:.1f}% < {threshold}%)")
...
def main():
    """Main entry point"""
    logger.info(f"🚀 Starting ULTIMATE #1 BOT")
