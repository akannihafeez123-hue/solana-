...
# Try Forex first, fallback to alternatives if needed
FOREX_ASSETS = ["EURUSD=X", "GBPUSD=X", "USDJPY=X", "AUDUSD=X", "USDCHF=X", "GC=F", "CL=F"]
ASSETS = FOREX_ASSETS  # Will be updated if Forex fails
...
# Add config for alternative assets
...
class RealDataProvider:
    ...
    def fetch_ohlcv(self, symbol: str, timeframe: str = "1d", limit: int = 300) -> Optional[pd.DataFrame]:
        ...
        try:
            ...
            # Try different symbol formats for Forex
            symbols_to_try = [symbol]
            if "=X" in symbol:
                # Try without =X suffix
                symbols_to_try.append(symbol.replace("=X", ""))
                # Try with FX prefix
                symbols_to_try.append(f"FX:{symbol.replace('=X', '')}")
...
def main_scan_loop():
    ...
    test_passed = False
    test_assets = ASSETS + ALTERNATIVE_ASSETS
    ...
    if not test_passed:
        ...
        logger.info("Switching to alternative assets...")
        # Switch to alternative assets
        ASSETS = ALTERNATIVE_ASSETS[:7]  # Use first 7 alternative assets
