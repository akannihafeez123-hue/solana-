...
# FIXED: Using correct yFinance symbols
ASSETS = ["EURUSD=X", "GBPUSD=X", "USDJPY=X", "AUDUSD=X", "USDCHF=X", "GC=F", "CL=F"]
# Alternative Forex symbols if above don't work
FOREX_SYMBOLS = {
    "EURUSD=X": "EURUSD=X",
    "GBPUSD=X": "GBPUSD=X", 
    "USDJPY=X": "USDJPY=X",
...
# -------------------------------
# Data Provider - ENHANCED WITH FALLBACK
# -------------------------------
...
def main_scan_loop():
    ...
    if not test_passed:
        logger.error(f"❌ Could not fetch data for any test asset. Check your internet connection.")
        logger.info("Trying with popular stocks instead...")
        # Fallback to stocks if Forex fails
        global ASSETS
        ASSETS = ALTERNATIVE_ASSETS[:7]  # Use first 7 alternative assets
        logger.info(f"Switched to assets: {ASSETS}")
