...
# USE RELIABLE STOCKS/ETFS (These ALWAYS work with yfinance)
RELIABLE_ASSETS = [
    "SPY",      # S&P 500 ETF - Most liquid ETF in the world
    "QQQ",      # NASDAQ 100 ETF
    "AAPL",     # Apple - Most valuable company
    "MSFT",     # Microsoft
...
# Use only reliable assets
ASSETS = RELIABLE_ASSETS[:7]  # Use first 7 assets
...
def compute_market_conditions(asset: str, df_map: Dict[str, pd.DataFrame]) -> Dict[str, float]:
    ...
    if not asset_config:
        asset_config = ASSET_CONFIG.get("SPY", {})  # Use SPY as default config
...
class RealDataProvider:
    ...
    def fetch_ohlcv(self, symbol: str, timeframe: str = "1d", limit: int = 300) -> Optional[pd.DataFrame]:
        ...
        try:
            ...
            # For stocks/ETFs, we don't need multiple symbol formats
            df = self.yf.download(
                symbol, 
                period=period, 
                interval=interval, 
...
def main_scan_loop():
    ...
    # SIMPLE TEST - Test with just SPY (always works)
    logger.info("🔧 Testing data fetching with SPY...")
    test_df = provider.fetch_ohlcv("SPY", "1h", limit=50)
    if test_df is not None and not test_df.empty:
        logger.info(f"✅ Data fetching works! SPY: {len(test_df)} bars fetched")
        logger.info(f"✅ Starting scanning of {len(ASSETS)} assets")
    else:
        logger.error("❌ CRITICAL: Could not fetch SPY data. Check internet connection.")
        logger.error("❌ The bot cannot continue without data.")
        return
    ...
    while True:
        try:
            ...
            for asset_idx, asset in enumerate(ASSETS):
                ...
                asset_config = ASSET_CONFIG.get(asset, {})
                if not asset_config:
                    # Use SPY config as default
                    asset_config = ASSET_CONFIG.get("SPY", {})
