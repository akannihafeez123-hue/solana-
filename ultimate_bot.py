...
# Try to get Alpha Vantage API key (free tier available)
ALPHA_VANTAGE_KEY = os.getenv("ALPHA_VANTAGE_KEY", "demo")  # 'demo' works for limited data
...
class MultiSourceDataProvider:
    """Data provider that tries multiple sources"""
    ...
    def _try_yfinance(self, symbol: str, interval: str, period: str) -> Optional[pd.DataFrame]:
        """Try yfinance first"""
        try:
    ...
    def _try_alphavantage(self, symbol: str, interval: str) -> Optional[pd.DataFrame]:
        """Try Alpha Vantage as fallback (free API)"""
        try:
            ...
            if response.status_code == 200:
                ...
                # Parse the data based on function
                if "Time Series (Daily)" in data:
                    time_series = data["Time Series (Daily)"]
                elif f"Time Series ({interval})" in data:
                    time_series = data[f"Time Series ({interval})"]
                else:
                    logger.debug(f"No time series data in Alpha Vantage response")
                    return None
    ...
    def _try_mock_data(self, symbol: str) -> Optional[pd.DataFrame]:
        """Generate mock data for testing when all else fails"""
        try:
            logger.warning(f"Using mock data for {symbol} - REAL TRADING DISABLED")
            ...
            # Add some trends
            df['close'] = df['close'] * (1 + np.arange(len(df)) * 0.0001)
            df['high'] = np.maximum(df['high'], df['close'] * 1.01)
            df['low'] = np.minimum(df['low'], df['close'] * 0.99)
    ...
    def fetch_ohlcv(self, symbol: str, timeframe: str = "1h", limit: int = 300) -> Optional[pd.DataFrame]:
        """Fetch OHLCV data with multiple fallback sources"""
        cache_key = f"{symbol}_{timeframe}"
        ...
        df = self._try_yfinance(symbol, interval, period)
        if df is not None:
            source = "yfinance"
        ...
        # TRY SOURCE 3: Mock data (for testing only - last resort)
        if df is None:
            df = self._try_mock_data(symbol)
            if df is not None:
                source = "MOCK DATA (Testing)"
                logger.warning("⚠️ USING MOCK DATA - Not for real trading!")
        ...
        if not isinstance(df, pd.DataFrame):
            logger.error(f"Data is not a DataFrame for {symbol}")
            return None
...
def main_scan_loop():
    ...
    if not test_passed:
        logger.warning("⚠️ Primary data sources failed, using fallback mode")
