...
def ensure_packages():
    pkgs = [
        ("yfinance", "yfinance==0.2.38"),  # Updated to latest
        ...
        ("alpha_vantage", "alpha_vantage==2.3.1"),
        ("pytz", "pytz==2023.3")
    ]
...
if ALPHA_VANTAGE_KEY == "demo":
    logger.warning("⚠️ Using Alpha Vantage demo key - limited data")
else:
    logger.info("✅ Using Alpha Vantage API key")
...
# -------------------------------
# FIXED MULTI-SOURCE DATA PROVIDER
# -------------------------------
class MultiSourceDataProvider:
    """Data provider that tries multiple sources with improved error handling"""
    ...
    def _try_yfinance_fixed(self, symbol: str, interval: str, period: str) -> Optional[pd.DataFrame]:
        """Fixed yfinance download with better error handling"""
        try:
            logger.debug(f"Trying yfinance for {symbol} {interval}")
            
            # Map intervals for yfinance
            yf_interval = interval
            if interval == "4h":
                yf_interval = "1h"  # Will resample
            elif interval == "1d":
                yf_interval = "1d"
            
            # Try with different ticker formats
            ticker_symbol = symbol
            if not ticker_symbol.endswith(".AX") and "." not in ticker_symbol:
                ticker_symbol = symbol  # Keep as is
            ...
            logger.debug(f"yfinance failed for {symbol}: {str(e)[:100]}")
        return None
    
    def _try_alphavantage_fixed(self, symbol: str, interval: str) -> Optional[pd.DataFrame]:
        """Fixed Alpha Vantage data fetch"""
        try:
            logger.debug(f"Trying Alpha Vantage for {symbol}")
            
            if ALPHA_VANTAGE_KEY == "demo":
                logger.debug("Using Alpha Vantage demo key - limited data")
            ...
            if response.status_code == 200:
                ...
                if not time_series_key:
                    logger.debug("No time series data in Alpha Vantage response")
                    return None
    ...
    def _try_alternative_source(self, symbol: str, interval: str) -> Optional[pd.DataFrame]:
        """Try alternative free APIs as backup"""
        try:
            # Try Yahoo Finance alternative method
            logger.debug(f"Trying alternative source for {symbol}")
            
            # Construct URL for Yahoo Finance CSV (old API)
            interval_map = {
                "1d": "1d",
                "1h": "1h",
                "15m": "15m",
            ...
            period = days_map.get(yf_interval, "1mo")
            ...
            try:
                ...
                if df is not None and not df.empty:
                    ...
                    if len(df) >= 20:
                        logger.info(f"✅ Alternative source success for {symbol}")
                        return df
                        
            except Exception as e:
                logger.debug(f"Alternative source failed: {str(e)[:100]}")
                
        except Exception as e:
            logger.debug(f"All alternative sources failed for {symbol}: {str(e)[:100]}")
        return None
    
    def fetch_ohlcv(self, symbol: str, timeframe: str = "1h", limit: int = 300) -> Optional[pd.DataFrame]:
        """Fetch OHLCV data with multiple fallback sources - FIXED"""
        cache_key = f"{symbol}_{timeframe}"
        ...
        # Map timeframe to interval/period
        interval_map = {
            "1d": "1d",
            "4h": "1h",  # Will resample
            "1h": "1h",
        ...
        # TRY SOURCE 1: Fixed yfinance
        df = self._try_yfinance_fixed(symbol, interval, period)
        if df is not None and not df.empty:
            source = "yfinance"
        
        # TRY SOURCE 2: Alpha Vantage (if yfinance fails)
        if (df is None or df.empty) and ALPHA_VANTAGE_KEY:
            df = self._try_alphavantage_fixed(symbol, interval)
            if df is not None and not df.empty:
                source = "Alpha Vantage"
        
        # TRY SOURCE 3: Alternative source
        if df is None or df.empty:
            df = self._try_alternative_source(symbol, interval)
            if df is not None and not df.empty:
                source = "Alternative API"
        
        if df is None or df.empty:
            logger.error(f"❌ All data sources failed for {symbol}")
            # Don't use mock data for real trading
            return None
        ...
        if timeframe == "4h" and interval == "1h" and len(df) > 0:
            try:
                ...
                # Resample to 4H with correct aggregation
...
class LegendaryTelegramBot:
    ...
    def format_alert(result: Dict) -> str:
        """Format legendary alert message - FIXED date format"""
...
# -------------------------------
# Enhanced Main Scan Loop with FIXED date format
# -------------------------------
def main_scan_loop():
    ...
    if not test_passed:
        logger.warning("⚠️ Primary data sources failed, trying alternative methods...")
