...
# Alternative Test Assets if Forex fails
ALTERNATIVE_ASSETS = [
    "SPY",  # S&P 500 ETF
    "QQQ",  # NASDAQ ETF
    "AAPL", # Apple
...
# SIMPLIFIED ASSETS - Use working symbols
# Forex pairs in yfinance work better as EURUSD=X, but sometimes need different format
# Let's use more reliable symbols
WORKING_ASSETS = [
    "EURUSD=X",  # Try this format first
    "GBPUSD=X",  
    "USDJPY=X",
    "AUDUSD=X",
    "USDCHF=X",
...
# Fallback to stocks/ETFs if Forex doesn't work
STOCK_ASSETS = [
...
# Start with WORKING_ASSETS, will switch to STOCK_ASSETS if needed
ASSETS = WORKING_ASSETS
...
def compute_market_conditions(asset: str, df_map: Dict[str, pd.DataFrame]) -> Dict[str, float]:
    ...
    if not asset_config:
        asset_config = ASSET_CONFIG.get("EURUSD=X", {})  # Use default config
...
class RealDataProvider:
    ...
    def fetch_ohlcv(self, symbol: str, timeframe: str = "1d", limit: int = 300) -> Optional[pd.DataFrame]:
        ...
        try:
            ...
            # Try different symbol formats for problematic symbols
            symbols_to_try = [symbol]
            
            # For Forex pairs, try different formats
            if "=X" in symbol:
                base_symbol = symbol.replace("=X", "")
                # Try with FX: prefix (works for some Forex)
                symbols_to_try.append(f"FX:{base_symbol}")
                # Try without =X (sometimes works)
                symbols_to_try.append(base_symbol)
                # Try with -CUR suffix (alternative format)
                symbols_to_try.append(f"{base_symbol}-CUR")
            ...
            for sym in symbols_to_try:
                try:
                    logger.debug(f"Trying symbol: {sym}")
                    df = self.yf.download(
                        sym, 
                        period=period, 
                        interval=interval, 
                        ...
                        auto_adjust=True,
                        timeout=15,
                        threads=False  # Disable threads for reliability
                    )
                    if df is not None and not df.empty:
                        successful_symbol = sym
                        logger.info(f"✅ Successfully fetched {sym} as {symbol}")
                        break
                except Exception as e:
            ...
            if df is None or df.empty:
                logger.warning(f"No data for {symbol} {timeframe} after trying {len(symbols_to_try)} symbol formats")
                return None
            ...
            logger.info(f"Fetched {symbol} {timeframe}: {len(df)} bars (via {successful_symbol})")
...
def main_scan_loop():
    ...
    global ASSETS  # Fixed: global declaration at the beginning of function
    ...
    # Test data fetching with multiple assets
    logger.info("🔧 Testing data fetching...")
    test_passed = False
    test_assets = ASSETS + STOCK_ASSETS  # Test both Forex and Stocks
    ...
    for test_asset in test_assets:
        ...
        if test_df is not None and not test_df.empty:
            ...
            test_passed = True
            # If a stock works, switch to stocks immediately
            if test_asset in STOCK_ASSETS and test_asset not in ASSETS:
                logger.info(f"Switching to stock assets since {test_asset} works")
                ASSETS = STOCK_ASSETS[:4]  # Use first 4 stock assets
                break
        else:
            logger.warning(f"✗ Could not fetch test data for {test_asset}")
    
    if not test_passed:
        logger.error("❌ Could not fetch data for any test asset. Check your internet connection.")
        logger.info("Switching to stock assets...")
        # Switch to stock assets
        ASSETS = STOCK_ASSETS[:4]  # Use first 4 stock assets
        logger.info(f"Now scanning: {ASSETS}")
    else:
        logger.info(f"✅ Data fetching test passed! Scanning {len(ASSETS)} assets")
    ...
    while True:
        try:
            ...
            for asset_idx, asset in enumerate(ASSETS):
                ...
                if not asset_config:
                    # Use default config
                    asset_config = ASSET_CONFIG.get("EURUSD=X", {})
