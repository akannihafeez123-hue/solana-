"""
📮 ULTIMATE LEGENDARY Pocket Option AI Scanner – V4.2
- Scanner loop runs as main process (foreground)
- Flask API for health/signals endpoints
- Telegram bot with async handlers (python-telegram-bot v20+)
- Advanced institutional strategies integrated with real logic
- Robust runtime installer for heavy deps (fallback; prefer build-time via requirements.txt)
"""
...
# -------------------------------
# Runtime installer (fallback; use requirements.txt in production)
# -------------------------------
def ensure_packages():
    pkgs = [
        "tensorflow==2.14.0",
    ...
    try:
        ...
        logger.warning(f"Failed to upgrade pip tools: {e}")
    for pkg in pkgs:
        ...
        try:
            ...
            # best-effort install; prefer binary wheels
            try:
                subprocess.check_call([sys.executable, "-m", "pip", "install", "--prefer-binary", "--no-cache-dir", pkg])
            except Exception as e:
                logger.error(f"Install failed for {pkg}: {e}")
...
class LegendaryTelegramBot:
    ...
    def start(self):
        if not TELEGRAM_AVAILABLE or not self.token or not self.chat_id:
            logger.warning("Telegram not configured/available; skipping bot start")
            return
        try:
    ...
    async def _start(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        try:
            ...
            if not is_admin(user_id):
                ...
                return
            await update.message.reply_text("📮 ULTIMATE Legendary Scanner Active (Scanner Mode)")
        except Exception as e:
...
def ema_macd_indicator(df: pd.DataFrame) -> Dict:
    ...
    try:
        ...
        if len(ema_fast) < 2 or len(macd_line) < 1 or len(macd_sig) < 1:
            return {"signal": "NEUTRAL", "score": 0.0, "reason": "EMA_MACD", "type": "indicator"}
...
# -------------------------------
# Main scanner loop (foreground)
# -------------------------------
def main_scan_loop():
    logger.info("📮 Initializing Scanner (Foreground Mode)")
    ...
    logger.info(f"🧠 TensorFlow: {'ENABLED' if TFK else 'FALLBACK'}")
    logger.info(f"🤖 Telegram: {'configured' if (TELEGRAM_AVAILABLE and TELEGRAM_TOKEN and CHAT_ID) else 'not configured'}")
            ...
            for asset in ASSETS:
                ...
                for expiry in EXPIRIES:
                    ...
                    if (result['signal'] != "NEUTRAL" and
                        ...
                        if TELEGRAM_AVAILABLE and telegram.app:
                            try:
...
if __name__ == "__main__":
    # Run scanner in foreground and Flask API in main thread
