...
# -------------------------------
# Telegram Bot (COMPLETELY REWRITTEN - SIMPLE APPROACH)
# -------------------------------
...
class LegendaryTelegramBot:
    ...
    def start(self):
        ...
        bot_thread = threading.Thread(target=self._run_bot, daemon=True, name="TelegramBotThread")
        bot_thread.start()
    ...
    async def _setup_and_run(self):
        ...
        try:
            ...
            # Add a handler for any message (for debugging)
    ...
    async def _echo(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        """Echo any non-command message (for debugging)"""
    ...
    async def _start(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        logger.info(f"📨 /start from user {update.effective_user.id} (@{update.effective_user.username})")
        
        if not is_admin(update.effective_user.id):
            logger.warning(f"⚠️ Unauthorized /start from {update.effective_user.id}")
            await update.message.reply_text("❌ Unauthorized. Your user ID is not in ADMIN_IDS.")
    ...
    async def _history(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        logger.info(f"📨 /history from user {update.effective_user.id}")
    ...
    async def _rejected(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        logger.info(f"📨 /rejected from user {update.effective_user.id}")
    ...
    async def _threshold(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        logger.info(f"📨 /threshold from user {update.effective_user.id}")
        ...
        if not args:
            await update.message.reply_text(
                "Usage:\n"
                "/threshold <value|reset> - Set global threshold\n"
                "/threshold <ASSET> <value|reset> - Set asset threshold\n\n"
                f"Current global: {GLOBAL_THRESHOLD:.1f}%"
            )
            return
        ...
        if len(args) == 1:
            ...
            try:
                ...
                await update.message.reply_text("Invalid value. Must be between 0 and 100.")
                return
    ...
    def send_signal_sync(self, alert_msg: str):
        ...
        if not self.app or not self.is_running:
            logger.warning("⚠️ Cannot send signal - bot not running")
            return
            
        try:
            # Get the running event loop from the bot thread
            loop = self.app.updater._loop if self.app.updater else None
            
            if not loop:
                logger.error("❌ No event loop available")
                return
            
            # Schedule coroutine on bot's event loop
