...
# Telegram setup - UPDATED VERSION WITH COMMAND HANDLERS
...
class AdvancedTelegramBot:
    def __init__(self, token: str, chat_id: str):
        ...
        try:
            ...
            # Add command handlers - ONLY START AND STATUS
            ...
            print("✅ Telegram bot connected successfully with command handlers")
...
<b>Status:</b> 🟢 ACTIVE
<b>Last Scan:</b> {datetime.utcnow().strftime('%Y-%m-%d %H:%M:%S')} UTC
<b>Assets Monitoring:</b> {len(ASSETS)}
<b>Confidence Threshold:</b> {LEGENDARY_GATE}%
<b>Scan Interval:</b> {SCAN_INTERVAL_SEC} seconds
<b>Signals Today:</b> {len([k for k in SIGNAL_HISTORY.keys() if k.startswith(datetime.utcnow().strftime('%Y%m%d'))])}
...
<b>Auto-scanning:</b> ✅ Background process running
    ...
    def get_recent_alerts(self):
        ...
        recent = list(SIGNAL_HISTORY.items())[-5:]  # Last 5 alerts
        if not recent:
    ...
    def send(self, text: str):
        ...
        if not self.available:
            print("📤 Telegram message (bot not available):")
            print(text)
            return
...
def main_scan_loop():
    ...
    telegram = AdvancedTelegramBot(TELEGRAM_TOKEN, CHAT_ID)
    
    print("✅ Scanner ready! Only alerts above 95% confidence will be sent.")
    print("🤖 Telegram bot listening for /start and /status commands")
...
if __name__ == "__main__":
    print("🚀 Starting ULTIMATE Legendary Scanner...")
    
    # Check environment
    if not TELEGRAM_TOKEN or not CHAT_ID:
        print("⚠️  WARNING: TELEGRAM_TOKEN or CHAT_ID not set in environment")
        print("   Telegram alerts will not work without these variables")
    ...
    if RUN_ONCE:
        ...
        print(f"🔄 Scanner will run in background every {SCAN_INTERVAL_SEC} seconds")
        print("🤖 Telegram bot listening for /start and /status commands")
