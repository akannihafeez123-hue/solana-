"""
📮 ULTIMATE LEGENDARY Pocket Option AI Scanner – V3.5 (MERGED & FIXED)
- Integrated advanced institutional strategies
- Fixed Telegram command responsiveness
- Safe async polling runner
- Memory leak prevention retained
"""
...
try:
    ...
    TFK = False
    logger.warning("TensorFlow/sklearn not available - running in fallback mode")
...
# FIX #1: Add missing ASSET_CONFIG
ASSET_CONFIG = {
    "EURUSD=X": {
        "min_data_points": 50,
...
# FIX #2: Limit signal history to prevent memory leaks
...
class LegendaryTelegramBot:
    ...
    def start(self):
        if not self.token or not self.chat_id:
            logger.warning("⚠️ Telegram not configured (missing TELEGRAM_TOKEN or CHAT_ID)")
            return
        try:
            ...
            # Run polling in its own thread via asyncio
...
# -------------------------------
# FIX #3: Safe validation helpers
# -------------------------------
def safe_series_check(series: pd.Series, min_len: int = 1) -> bool:
    """Safe validation of pandas series"""
...
def safe_df_check(df: pd.DataFrame, min_len: int = 1) -> bool:
    """Safe validation of pandas dataframe"""
...
def smart_money_filter(df: pd.DataFrame, asset_config: Dict) -> Dict:
    ...
    try:
        ...
        logger.debug(f"Smart money filter error: {e}")
        return {"signal": "NEUTRAL", "score": 0.0, "reason": "SmartMoney", "type": "filter"}
...
def vwap_deviation_filter(df: pd.DataFrame, asset_config: Dict) -> Dict:
    ...
    try:
        ...
        logger.debug(f"VWAP filter error: {e}")
        return {"signal": "NEUTRAL", "score": 0.0, "reason": "VWAP", "type": "filter"}
...
def volume_profile_filter(df: pd.DataFrame, asset_config: Dict) -> Dict:
    ...
    try:
        ...
        logger.debug(f"Volume profile filter error: {e}")
        return {"signal": "NEUTRAL", "score": 0.0, "reason": "VolProfile", "type": "filter"}

# -------------------------------
# Core strategies (existing)
# -------------------------------
def fibonacci_vortex_hidden(df: pd.DataFrame) -> Dict:
    ...
    try:
        ...
        logger.debug(f"Fibonacci vortex error: {e}")
        return {"signal": "NEUTRAL", "score": 0.0, "reason": "FibVortex", "type": "strategy"}
...
def dark_pool_institutional_hidden(df: pd.DataFrame) -> Dict:
    ...
    try:
        ...
        logger.debug(f"Dark pool error: {e}")
        return {"signal": "NEUTRAL", "score": 0.0, "reason": "DarkPool", "type": "strategy"}
...
def break_of_structure(df: pd.DataFrame) -> Dict:
    ...
    try:
        ...
        logger.debug(f"Break of structure error: {e}")
        return {"signal": "NEUTRAL", "score": 0.0, "reason": "BOS", "type": "strategy"}
...
def fair_value_gap(df: pd.DataFrame) -> Dict:
    ...
    try:
        ...
        logger.debug(f"Fair value gap error: {e}")
        return {"signal": "NEUTRAL", "score": 0.0, "reason": "FVG", "type": "strategy"}
...
def ema_macd_indicator(df: pd.DataFrame) -> Dict:
    ...
    try:
        ...
        logger.debug(f"EMA MACD error: {e}")
        return {"signal": "NEUTRAL", "score": 0.0, "reason": "EMA_MACD", "type": "indicator"}
...
def volume_indicator(df: pd.DataFrame, asset_config: Dict = None) -> Dict:
    ...
    try:
        ...
        logger.debug(f"Volume indicator error: {e}")
        return {"signal": "NEUTRAL", "score": 0.0, "reason": "Volume", "type": "indicator"}
...
def supertrend_bollinger_indicator(df: pd.DataFrame) -> Dict:
    ...
    try:
        ...
        logger.debug(f"SuperTrend+Bollinger error: {e}")
        return {"signal": "NEUTRAL", "score": 0.0, "reason": "SuperTrend+Boll", "type": "indicator"}

# -------------------------------
# Advanced institutional strategies (integrated)
# -------------------------------
...
def gamma_flip_detector(df: pd.DataFrame) -> Dict:
    ...
    try:
        ...
        maxima_idx = _argrelextrema(close_prices, np.greater, order=10)[0]
        minima_idx = _argrelextrema(close_prices, np.less, order=10)[0]
...
def auction_theory_analyzer(df: pd.DataFrame) -> Dict:
    ...
    try:
        ...
        poc_price = base_low + poc_tick * tick_size
...
# Note: correlation_regime_strategy requires multi-asset market_dfs input.
# We include it for future extension but do NOT add it to engines to avoid signature mismatch.
def correlation_regime_strategy(df: pd.DataFrame, market_dfs: Dict[str, pd.DataFrame]) -> Dict:
    if df is None or len(df) < 100:
        return {"signal": "NEUTRAL", "score": 0.0, "reason": "CorrelationRegime", "type": "strategy"}
    try:
        if not market_dfs or len(market_dfs) == 0:
            return {"signal": "NEUTRAL", "score": 0.0, "reason": "CorrelationRegime", "type": "strategy"}
        primary_returns = df['close'].pct_change().iloc[-50:]
        correlations = []
        for asset_name, asset_df in market_dfs.items():
            if asset_df is not None and len(asset_df) >= 50:
                asset_returns = asset_df['close'].pct_change().iloc[-50:]
                if len(asset_returns) == len(primary_returns):
                    corr = primary_returns.corr(asset_returns)
                    correlations.append(corr)
        if len(correlations) < 2:
            return {"signal": "NEUTRAL", "score": 0.0, "reason": "CorrelationRegime", "type": "strategy"}
        avg_correlation = np.mean(correlations)
        historical_correlation = 0.7
        if avg_correlation < 0.3 and historical_correlation > 0.6:
            momentum = (df['close'].iloc[-1] - df['close'].iloc[-10]) / (df['close'].iloc[-10] + 1e-9)
            if abs(momentum) > 0.02:
                if momentum > 0:
                    return {"signal": "BUY", "score": 94.0, "reason": "CorrelationRegime", "type": "strategy"}
                else:
                    return {"signal": "SELL", "score": 94.0, "reason": "CorrelationRegime", "type": "strategy"}
        return {"signal": "NEUTRAL", "score": 0.0, "reason": "CorrelationRegime", "type": "strategy"}
    except Exception:
        return {"signal": "NEUTRAL", "score": 0.0, "reason": "CorrelationRegime", "type": "strategy"}
...
ALL_STRATEGIES = [
    ...
    # Advanced ones included in the global pool too (nice for stats/logging)
    market_microstructure_imbalance, liquidity_void_hunter, volatility_regime_detector,
    gamma_flip_detector, fractal_dimension_strategy, institutional_footprint_detector,
    auction_theory_analyzer, spoofing_detector
]
ALL_INDICATORS = [
...
def engine_ai_analysis(asset: str, tf_data: Dict[str, pd.DataFrame], engine: Dict) -> Dict:
    ...
    for tf, df in tf_data.items():
        ...
        for strategy_fn in engine["strategies"]:
            try:
                # Only call strategies with single-arg signature (df)
...
# -------------------------------
def main_scan_loop():
    logger.info("📮 Initializing ULTIMATE Legendary Scanner (Merged & Fixed)")
    logger.info(f"⚡ Engines: Quantum / Momentum / Breakout / MeanReversion")
    logger.info(f"📊 Base Pools – Strategies: {len(ALL_STRATEGIES)} | Indicators: {len(ALL_INDICATORS)}")
            ...
            for asset in ASSETS:
                ...
                for tf in ALL_TIMEFRAMES[:4]:
                    try:
                        ...
                        if safe_df_check(df, ASSET_CONFIG.get(asset, {}).get("min_data_points", 50)):
                            tf_data[tf] = df
                    except Exception as e:
