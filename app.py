"""
📮 ULTIMATE LEGENDARY Pocket Option AI Scanner – V5.0
- Runtime dependency installer for heavy packages (pip-bootstrapped)
- Telegram bot starts immediately at app startup (async polling in its own thread)
...
# -------------------------------
def ensure_packages():
    # Minimal logging setup for installer stage
    logging.basicConfig(level=logging.INFO)
    inst_logger = logging.getLogger("installer")
    pkgs = [
        "tensorflow==2.14.0",
        "scikit-learn==1.3.2",
        "scipy==1.11.3",
    ...
    try:
        ...
        subprocess_check([sys.executable, "-m", "pip", "install", "--upgrade", "pip", "setuptools", "wheel"])
    except Exception as e:
        inst_logger.warning(f"Failed to upgrade pip tools: {e}")
    for pkg in pkgs:
        mod = pkg.split("==")[0].replace("-", "_")
        try:
            __import__(mod)
        except Exception:
            inst_logger.info(f"📦 Installing {pkg}...")
            try:
                __import__("subprocess").check_call([sys.executable, "-m", "pip", "install", "--prefer-binary", "--no-cache-dir", pkg])
            except Exception as e:
                inst_logger.warning(f"Install attempt failed for {pkg}: {e}")

ensure_packages()

# -------------------------------
# Logging setup (app level)
# -------------------------------
logging.basicConfig(
    level=logging.INFO,
...
def market_microstructure_imbalance(df: pd.DataFrame) -> Dict:
    ...
    ib = (buying_pressure.iloc[-1] if not np.isnan(buying_pressure.iloc[-1]) else 0.0) - (selling_pressure.iloc[-1] if not np.isnan(selling_pressure.iloc[-1]) else 0.0)
    if current_impact < avg_impact * 0.7 and ib > 0.15:
        return {"signal": "BUY", "score": 97.0, "reason": "MicrostructureImbalance", "type": "strategy"}
    elif current_impact < avg_impact * 0.7 and ib < -0.15:
...
# Main scanner loop
# -------------------------------
telegram_instance: Optional[LegendaryTelegramBot] = None
