...
# TensorFlow/Keras optional (for AI training). In Actions, we skip TF due to build weight.
try:
...
# ------------------------------------------------------------------------------------
# AI core (lightweight; TF optional with fallback)
# ------------------------------------------------------------------------------------
class AdvancedKerasAI:
    def __init__(self, lookback: int = 100, expiry_minutes: int = 60):
        ...
        self.scaler = StandardScaler() if (TFK and StandardScaler is not None) else None

    def build_model(self, input_shape: Tuple[int, int]) -> Optional[Model]:
        if not TFK: return None
    ...
    def train(self, df: pd.DataFrame, epochs: int = 20, validation_split: float = 0.2) -> bool:
        if not TFK or len(df) < self.lookback + 10: return False
    ...
    def save_model(self, path: str):
        if self.model is not None and TFK:
            self.model.save(path)

    def load_model(self, path: str) -> bool:
        if not TFK: return False
        try:
