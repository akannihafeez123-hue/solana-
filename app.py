"""
🔮 ULTIMATE LEGENDARY Pocket Option AI Scanner — Per-Asset Tuned V2.2
Auto mode selection + institutional filters + Telegram alerts (default ≥95%)
Adds /threshold Telegram command to set global or per-asset alert thresholds
"""
...
def install_tensorflow_runtime():
    try:
        ...
        try:
            subprocess.check_call([sys.executable, "-m", "pip", "install", "--quiet", "tensorflow==2.14.0"])
            import tensorflow as tf
            print(f"✅ TensorFlow installed successfully! (v{tf.__version__})")
            return True
        except Exception as e:
            print(f"⚠️ TensorFlow install failed: {e}")
            return False
...
def parse_admin_ids():
    ...
    if admin_ids_str:
        for x in admin_ids_str.split(","):
            ...
            if x.isdigit():
                ids.append(int(x))
    if not ids and CHAT_ID and CHAT_ID.isdigit():
        ids = [int(CHAT_ID)]
    return ids
...
WEBHOOK_SECRET = os.getenv("WEBHOOK_SECRET", "change_webhook")
...
ASSET_CONFIG: Dict[str, Dict] = {
    "EURUSD=X": {"volatility_threshold": 0.6, "trend_strength_multiplier": 1.0, "momentum_volume_multiplier": 1.8, "vwap_threshold": 0.008, "volume_profile_sensitivity": 0.005, "min_data_points": 60},
    "GBPUSD=X": {"volatility_threshold": 0.8, "trend_strength_multiplier": 1.1, "momentum_volume_multiplier": 1.9, "vwap_threshold": 0.009, "volume_profile_sensitivity": 0.006, "min_data_points": 60},
    "USDJPY=X": {"volatility_threshold": 0.7, "trend_strength_multiplier": 0.95, "momentum_volume_multiplier": 1.7, "vwap_threshold": 0.007, "volume_profile_sensitivity": 0.005, "min_data_points": 60},
    "AUDUSD=X": {"volatility_threshold": 0.9, "trend_strength_multiplier": 1.05, "momentum_volume_multiplier": 2.0, "vwap_threshold": 0.01, "volume_profile_sensitivity": 0.007, "min_data_points": 60},
    "GC=F":     {"volatility_threshold": 1.5, "trend_strength_multiplier": 1.2, "momentum_volume_multiplier": 2.2, "vwap_threshold": 0.015, "volume_profile_sensitivity": 0.01, "min_data_points": 80}
}

# -------------------------------
# Threshold overrides (global and per-asset)
# -------------------------------
GLOBAL_THRESHOLD = LEGENDARY_GATE  # default global threshold (95)
ASSET_THRESHOLD_OVERRIDE: Dict[str, float] = {}  # e.g., {"EURUSD=X": 90.0}
...
class LegendaryTelegramBot:
    def __init__(self, token: str, chat_id: str):
        ...
        if self.available:
            ...
            dp.add_handler(CommandHandler("threshold", self._threshold))  # new command
    ...
    def _status(self, update, context):
        if not AdminAuth.verify_admin(update.effective_user.id):
            return
        # show global threshold and per-asset overrides
    ...
    def _threshold(self, update, context):
        """
        Usage:
          /threshold 90                -> set global threshold to 90%
          /threshold EURUSD=X 90      -> set EURUSD=X threshold to 90%
          /threshold reset            -> reset global threshold to default (95)
          /threshold EURUSD=X reset   -> remove per-asset override for EURUSD=X
        """
        if not AdminAuth.verify_admin(update.effective_user.id):
            update.message.reply_text("❌ Unauthorized")
            return
        ...
        if not args:
            update.message.reply_text("Usage: /threshold <value|reset> or /threshold <ASSET> <value|reset>")
            return
        ...
        if len(args) == 1:
            ...
            if token == "reset":
                ...
                update.message.reply_text(f"🔁 Global threshold reset to default {GLOBAL_THRESHOLD}%")
                return
            try:
                val = float(args[0])
                if val <= 0 or val > 100:
                ...
                update.message.reply_text(f"✅ Global threshold set to {GLOBAL_THRESHOLD:.1f}%")
                return
            except Exception:
                update.message.reply_text("Invalid value. Provide a number between 1 and 100, or 'reset'.")
                return
        ...
        if len(args) >= 2:
            ...
            if asset not in ASSETS:
                update.message.reply_text(f"Unknown asset: {asset}. Supported: {', '.join(ASSETS)}")
                return
            if token == "reset":
                if asset in ASSET_THRESHOLD_OVERRIDE:
                    del ASSET_THRESHOLD_OVERRIDE[asset]
                    update.message.reply_text(f"🔁 Threshold override removed for {asset}. Now uses global {GLOBAL_THRESHOLD:.1f}%")
                else:
                    update.message.reply_text(f"No override set for {asset}. Global {GLOBAL_THRESHOLD:.1f}% applies.")
                return
            try:
                val = float(args[1])
                if val <= 0 or val > 100:
                ...
                update.message.reply_text(f"✅ Threshold for {asset} set to {val:.1f}% (alerts >= {val:.1f}%)")
                return
            except Exception:
                update.message.reply_text("Invalid value. Provide a number between 1 and 100, or 'reset'.")
                return
...
# -------------------------------
# Institutional & SMC strategies (use asset_config)
# -------------------------------
...
# -------------------------------
# Market regime detection (uses asset_config)
# -------------------------------
def compute_market_conditions(asset: str, df_map: Dict[str, pd.DataFrame]) -> Dict[str, float]:
    ...
    return {"volatility": volatility, "trend_strength": trend_strength, "momentum_spike": momentum_spike, "breakout_detected": breakout_detected, "range_bound": range_bound}
...
# -------------------------------
# Core analysis per engine (passes asset_config to filters/indicators)
# -------------------------------
...
def format_ultimate_alert(result: Dict, engine_name: str) -> str:
    ...
    return f"""
🔮 <b>ULTIMATE LEGENDARY SIGNAL</b>  <i>[{engine_name}]</i>
...
# -------------------------------
def main_scan_loop():
    print("🔮 Initializing ULTIMATE Legendary Scanner (Per-Asset Tuned + /threshold)...")
        ...
        try:
            ...
            for asset in ASSETS:
                ...
                for expiry in EXPIRIES:
                    ...
                    # determine threshold: per-asset override else global
