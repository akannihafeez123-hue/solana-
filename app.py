...
# Assets: Yahoo-style names retained for your bot, mapped internally to Oanda/Binance
...
ASSET_CONFIG = {
    "EURUSD=X": {"min_data_points": 50, "volatility_threshold": 0.35, "vwap_threshold": 0.008,
                 "volume_profile_sensitivity": 0.004, "trend_strength_multiplier": 1.2,
                 "momentum_volume_multiplier": 1.8, "min_volume_ratio": 0.7,
                 "max_spread_pct": 0.0005, "choppy_threshold": 0.3},
    "GBPUSD=X": {"min_data_points": 50, "volatility_threshold": 0.4, "vwap_threshold": 0.01,
                 "volume_profile_sensitivity": 0.005, "trend_strength_multiplier": 1.3,
                 "momentum_volume_multiplier": 2.0, "min_volume_ratio": 0.7,
                 "max_spread_pct": 0.0006, "choppy_threshold": 0.35},
    "USDJPY=X": {"min_data_points": 50, "volatility_threshold": 0.38, "vwap_threshold": 0.009,
                 "volume_profile_sensitivity": 0.0045, "trend_strength_multiplier": 1.15,
                 "momentum_volume_multiplier": 1.9, "min_volume_ratio": 0.65,
                 "max_spread_pct": 0.0005, "choppy_threshold": 0.32},
    "AUDUSD=X": {"min_data_points": 50, "volatility_threshold": 0.42, "vwap_threshold": 0.01,
                 "volume_profile_sensitivity": 0.005, "trend_strength_multiplier": 1.25,
                 "momentum_volume_multiplier": 2.0, "min_volume_ratio": 0.65,
                 "max_spread_pct": 0.0007, "choppy_threshold": 0.35},
    "USDCHF=X": {"min_data_points": 50, "volatility_threshold": 0.36, "vwap_threshold": 0.009,
                 "volume_profile_sensitivity": 0.0045, "trend_strength_multiplier": 1.2,
                 "momentum_volume_multiplier": 1.85, "min_volume_ratio": 0.65,
                 "max_spread_pct": 0.0006, "choppy_threshold": 0.33},
    "GC=F": {"min_data_points": 50, "volatility_threshold": 0.5, "vwap_threshold": 0.012,
             "volume_profile_sensitivity": 0.006, "trend_strength_multiplier": 1.4,
             "momentum_volume_multiplier": 2.2, "min_volume_ratio": 0.75,
             "max_spread_pct": 0.001, "choppy_threshold": 0.4},
    "CL=F": {"min_data_points": 50, "volatility_threshold": 0.55, "vwap_threshold": 0.015,
             "volume_profile_sensitivity": 0.007, "trend_strength_multiplier": 1.5,
             "momentum_volume_multiplier": 2.3, "min_volume_ratio": 0.75,
             "max_spread_pct": 0.0012, "choppy_threshold": 0.45}
}
...
# -------------------------------
# ADVANCED INSTITUTIONAL STRATEGIES (subset as in your file)
# -------------------------------
def market_microstructure_imbalance(df: pd.DataFrame) -> Dict:
    ...
    try:
        ...
        buying = ((df['close'] - df['low']) / (df['high'] - df['low'] + 1e-9)).rolling(10).mean()
        selling = ((df['high'] - df['close']) / (df['high'] - df['low'] + 1e-9)).rolling(10).mean()
        imbalance = buying.iloc[-1] - selling.iloc[-1]
...
def volatility_regime_detector(df: pd.DataFrame) -> Dict:
    ...
    try:
        ...
        vol_acc = vol_of_vol.iloc[-1] / vol_of_vol.mean()
        if vol_zscore < -0.5 and recent_vol_change > 1.3 and vol_acc > 1.2:
            pm = (df['close'].iloc[-1] - df['close'].iloc[-5]) / df['close'].iloc[-5]
            return {"signal": "BUY" if pm > 0 else "SELL", "score": 93.0, "reason": "VolRegime", "type": "strategy"}
        elif vol_zscore > 1.0 and recent_vol_change < 0.8:
            price_z = (df['close'].iloc[-1] - df['close'].rolling(50).mean().iloc[-1]) / (df['close'].rolling(50).std().iloc[-1] + 1e-9)
            if price_z > 1.5:
...
def market_entropy_strategy(df: pd.DataFrame) -> Dict:
    ...
    try:
        ...
        if normalized_entropy < 0.7 and trend_strength > 0.015:
            return {"signal": "BUY" if ema_short.iloc[-1] > ema_long.iloc[-1] else "SELL",
                    "score": 92.0, "reason": "MarketEntropy", "type": "strategy"}
...
# -------------------------------
# CLASSIC STRATEGIES (as in your file)
# -------------------------------
...
class RealDataProvider:
    ...
    OANDA_INSTRUMENTS = {
        ...
        "AUDUSD=X": "AUD_USD",
        "USDCHF=X": "USD_CHF",
        "GC=F": "XAU_USD",    # Gold CFD
        "CL=F": "WTICO_USD"   # WTI Crude Oil CFD
    }

    BINANCE_SYMBOLS = {
        # Forex proxies and commodities (if you want them from Binance instead)
        "EURUSD=X": "EURUSDT",
        "GBPUSD=X": "GBPUSDT",
        "AUDUSD=X": "AUDUSDT",
        "GC=F": "XAUUSDT",
        # Oil not reliably available via Binance spot; prefer Oanda CFD for CL=F
    }

    def __init__(self,
                 prefer_oanda_for: Optional[List[str]] = None,
                 prefer_binance_for: Optional[List[str]] = None):
        # Preferences: default to Oanda for forex and CFDs
                ...
                for expiry in EXPIRIES:
                    ...
                    if (result['confidence'] >= threshold and
                        ...
                        if telegram.app:
                            try:
                                import asyncio
                                asyncio.run(telegram.app.bot.send_message(chat_id=CHAT_ID, text=alert_msg, parse_mode="HTML"))
                            except Exception as e:
