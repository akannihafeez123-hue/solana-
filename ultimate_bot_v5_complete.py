...
ULTIMATE ALL-IN-ONE VERSION - Optimized for Replit with Keepalive
"""
...
# -------------------------------
# AUTO-INSTALL DEPENDENCIES (Replit compatible)
# -------------------------------
...
def install_missing_packages():
    ...
    if missing_packages:
        ...
        try:
            ...
            # Relaunch script after installation
...
if not install_missing_packages():
    print("⚠️ Some packages may be missing. The bot may not work properly.")
...
KEEPALIVE_INTERVAL = int(os.getenv("KEEPALIVE_INTERVAL", "300"))  # 5 minutes
UPTIME_ROBOT_ENABLED = os.getenv("UPTIME_ROBOT_ENABLED", "False") == "True"
UPTIME_ROBOT_URL = os.getenv("UPTIME_ROBOT_URL", "")
HEALTHCHECKS_ENABLED = os.getenv("HEALTHCHECKS_ENABLED", "False") == "True"
...
class KeepaliveManager:
    """Manage keepalive pings to prevent Replit/Heroku sleep"""
    ...
    def __init__(self):
        ...
        self.session.headers.update({
            'User-Agent': 'UltimateTradingBot/5.0',
            'Accept': 'application/json',
        })
        
    def ping_all(self):
        """Send pings to all configured keepalive services"""
        self.last_ping = datetime.utcnow()
        self.ping_count += 1
        
        results = []
        
        # Primary Keepalive URL
        if KEEPALIVE_ENABLED and KEEPALIVE_URL:
            try:
                response = self.session.get(KEEPALIVE_URL, timeout=10)
                if response.status_code in [200, 201, 202, 204]:
                    logger.info(f"✅ Keepalive ping #{self.ping_count} successful")
                    results.append(("Primary Keepalive", True))
                else:
                    logger.warning(f"⚠️ Keepalive ping failed: HTTP {response.status_code}")
                    results.append(("Primary Keepalive", False))
                    self.fail_count += 1
            except Exception as e:
                logger.warning(f"⚠️ Keepalive ping error: {str(e)[:100]}")
                results.append(("Primary Keepalive", False))
                self.fail_count += 1
        
        # Uptime Robot
        if UPTIME_ROBOT_ENABLED and UPTIME_ROBOT_URL:
            try:
                response = self.session.get(UPTIME_ROBOT_URL, timeout=10)
                if response.status_code in [200, 201, 202, 204]:
                    results.append(("Uptime Robot", True))
                else:
                    results.append(("Uptime Robot", False))
            except Exception:
                results.append(("Uptime Robot", False))
        
        # Healthchecks.io
        if HEALTHCHECKS_ENABLED and HEALTHCHECKS_URL:
            try:
                response = self.session.get(HEALTHCHECKS_URL, timeout=10)
                if response.status_code in [200, 201, 202, 204]:
                    results.append(("Healthchecks.io", True))
                else:
                    results.append(("Healthchecks.io", False))
            except Exception:
                results.append(("Healthchecks.io", False))
        ...
        if results:
            ...
            logger.info(f"📡 Keepalive summary: {success_count}/{len(results)} services successful")
    ...
    def get_stats(self):
        ...
        return {
            ...
            "services": {
                "primary": bool(KEEPALIVE_URL),
                "uptime_robot": bool(UPTIME_ROBOT_URL),
...
def keepalive_worker():
    """Background thread to send keepalive pings"""
    last_keepalive = None
    keepalive_counter = 0
        ...
        try:
            ...
            if last_keepalive is None or (now - last_keepalive).total_seconds() >= KEEPALIVE_INTERVAL:
                if KEEPALIVE_ENABLED:
                    keepalive_counter += 1
                    logger.info(f"📡 Sending keepalive ping #{keepalive_counter}...")
                    results = keepalive_manager.ping_all()
                    
                    # Log summary
                    success_count = sum(1 for _, success in results if success)
                    total_services = len(results)
                    
                    if success_count == total_services and total_services > 0:
                        logger.info(f"✅ Keepalive successful: {success_count}/{total_services} services")
                    elif total_services > 0:
                        logger.warning(f"⚠️ Keepalive partial: {success_count}/{total_services} services")
            ...
            logger.error(f"❌ Keepalive worker error: {str(e)}")
...
if FLASK_AVAILABLE:
    ...
    def health_check():
        ...
        # Send keepalive if requested
        if request.args.get('ping') == 'true' and KEEPALIVE_ENABLED:
            results = keepalive_manager.ping_all()
            keepalive_status = {
                "ping_sent": True,
                "results": [{"service": name, "success": success} for name, success in results]
            }
        else:
            keepalive_status = {"ping_sent": False}
        
        return jsonify({
            "status": "legendary",
            "timestamp": now.isoformat(),
            "uptime": str(now - bot_start_time) if 'bot_start_time' in globals() else "unknown",
            ...
            "keepalive": keepalive_manager.get_stats(),
    ...
    @app.route('/keepalive_test', methods=['GET'])
    def keepalive_test():
        """Test keepalive connections"""
        if not KEEPALIVE_ENABLED:
            return jsonify({"success": False, "message": "Keepalive not enabled"})
        ...
        return jsonify({
            "success": True,
            "message": f"Keepalive test completed: {success_count}/{len(results)} successful",
            "results": [{"service": name, "success": success} for name, success in results]
        })
...
def main_scan_loop():
    ...
    if KEEPALIVE_ENABLED:
        print(f"   ✅ ENABLED")
        if KEEPALIVE_URL:
            print(f"   Primary: {KEEPALIVE_URL[:50]}...")
        if UPTIME_ROBOT_URL:
            print(f"   Uptime Robot: Configured")
        if HEALTHCHECKS_URL:
            print(f"   Healthchecks.io: Configured")
        print(f"   Interval: {KEEPALIVE_INTERVAL} seconds")
    else:
        print(f"   ❌ DISABLED")
    ...
    print(f"\n🌐 Web Interface: {'ENABLED' if FLASK_AVAILABLE else 'DISABLED'}")
    print("="*70)
    ...
    last_keepalive_check = datetime.utcnow()
    ...
    while True:
        try:
            ...
            # Send periodic keepalive pings from main thread too
            if KEEPALIVE_ENABLED and (now_utc - last_keepalive_check).total_seconds() >= KEEPALIVE_INTERVAL:
                logger.info(f"📡 Sending keepalive ping...")
                keepalive_manager.ping_all()
                last_keepalive_check = now_utc
...
def main():
    ...
    print("✅ Keepalive System for 24/7 Uptime")
    ...
    print(f"   Keepalive: {'✅ Enabled' if KEEPALIVE_ENABLED else '❌ Disabled'}")
    print(f"   Port: {PORT}")
    ...
    # Keepalive setup instructions
    if not KEEPALIVE_ENABLED:
        print(f"\n⚠️ Keepalive not enabled")
        print(f"   To prevent Replit/Heroku from sleeping:")
        print(f"   1. Set KEEPALIVE_ENABLED=True in .env")
        print(f"   2. Set KEEPALIVE_URL to your keepalive service URL")
        print(f"   3. Optionally configure UPTIME_ROBOT_URL or HEALTHCHECKS_URL")
    
    # Start keepalive worker thread
    if KEEPALIVE_ENABLED:
        keepalive_thread = threading.Thread(target=keepalive_worker, daemon=True)
        keepalive_thread.start()
        logger.info("✅ Keepalive worker started")
        print(f"\n📡 Keepalive system: ACTIVE (every {KEEPALIVE_INTERVAL} seconds)")
    else:
        logger.info("⚠️ Keepalive system disabled")
        print(f"\n📡 Keepalive system: DISABLED")
    ...
    if FLASK_AVAILABLE:
        ...
        print(f"   Test keepalive: http://localhost:{PORT}/keepalive_test")
    else:
