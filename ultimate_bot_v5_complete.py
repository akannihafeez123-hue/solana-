...
try:
    ...
    try:
        ...
        print("⚠️ TA-Lib not available. Using built-in indicators.")
...
# UTILITY FUNCTIONS
# -------------------------------
def is_valid_dataframe(df: pd.DataFrame, min_len: int = 20) -> bool:
    """Safely check if DataFrame is valid and has enough data"""
    if df is None:
        return False
...
class AdvancedIndicators:
    ...
    def calculate_ema(series: pd.Series, period: int) -> pd.Series:
        ...
        try:
            if not is_valid_dataframe(pd.DataFrame({'close': series}), period):
    ...
    def calculate_rsi(series: pd.Series, period: int = 14) -> pd.Series:
        """Calculate RSI"""
        try:
            if not is_valid_dataframe(pd.DataFrame({'close': series}), period + 1):
                return pd.Series(dtype=float)
    ...
    def calculate_macd(series: pd.Series) -> Tuple[pd.Series, pd.Series, pd.Series]:
        """Calculate MACD"""
        try:
            if not is_valid_dataframe(pd.DataFrame({'close': series}), 26):
    ...
    def calculate_atr(high: pd.Series, low: pd.Series, close: pd.Series, period: int = 14) -> pd.Series:
        ...
        try:
            df_check = pd.DataFrame({'high': high, 'low': low, 'close': close})
            if not is_valid_dataframe(df_check, period):
                return pd.Series(dtype=float)
    ...
    def calculate_supertrend(df, period=10, multiplier=3):
        ...
        try:
            if not is_valid_dataframe(df, period):
                return None, None
            
            hl2 = (df['high'] + df['low']) / 2
            atr = AdvancedIndicators.calculate_atr(df['high'], df['low'], df['close'], period)
    ...
    def calculate_order_blocks(df, lookback=20):
        ...
        try:
            if not is_valid_dataframe(df, lookback * 2):
                return []
            ...
            for i in range(lookback, len(df)-lookback):
                ...
                volume_window = df['volume'].iloc[i-lookback:i]
                if len(volume_window) == 0:
                    continue
                    
                volume_ratio = df['volume'].iloc[i] / volume_window.mean() if volume_window.mean() > 0 else 1
        ...
        try:
            ...
            for i in range(1, len(df)-1):
                ...
                if current_low > prev_high and next_low > current_low:
                    gap_size = (current_low - prev_high) / prev_high if prev_high > 0 else 0
                ...
                # Bearish FVG
                    ...
                    gap_size = (prev_low - current_high) / current_high if current_high > 0 else 0
                    if gap_size > threshold:
                        fvgs.append({
                            'index': i,
                            'type': 'BEARISH',
    ...
    def calculate_fibonacci_levels(df):
        ...
        try:
            if not is_valid_dataframe(df, 50):
                return {}, '0', 0
    ...
    def calculate_elliott_waves(df):
        ...
        try:
            if not is_valid_dataframe(df, 30):
            ...
            if len(all_points) >= 5:
                ...
                # Basic wave rules
                wave2_retrace = abs(waves[1][2] - waves[0][2]) / abs(waves[0][2] - waves[1][2]) if abs(waves[0][2] - waves[1][2]) > 0 else 0
                wave4_retrace = abs(waves[3][2] - waves[2][2]) / abs(waves[2][2] - waves[3][2]) if abs(waves[2][2] - waves[3][2]) > 0 else 0
    ...
    def quantum_resonance_analysis(df, frequency=0.05):
        ...
        try:
            ...
            normalized_resonance = resonance / (n * np.std(prices)) if np.std(prices) > 0 else 0
    ...
    def detect_dark_pool_activity(df, threshold=3):
        ...
        try:
            ...
            recent_volume = df['volume'].rolling(20).mean().iloc[-1] if len(df) >= 20 else df['volume'].mean()
            current_volume = df['volume'].iloc[-1]
            volume_ratio = current_volume / recent_volume if recent_volume > 0 else 1
            
            price_change = abs(df['close'].iloc[-1] - df['close'].iloc[-2]) / df['close'].iloc[-2] if len(df) >= 2 and df['close'].iloc[-2] > 0 else 0
            
            # Calculate order imbalance
            buy_mask = df['close'] > df['open']
            sell_mask = df['close'] < df['open']
            
            buy_volume = df.loc[buy_mask, 'volume'].sum()
            sell_volume = df.loc[sell_mask, 'volume'].sum()
...
class QuantumStrategies:
    ...
    def order_block_detection(df):
        ...
        try:
            if not is_valid_dataframe(df, 100):
                ...
                if latest_block['is_bullish'] and current_price > latest_block['price']:
                    return {
                        ...
                        "details": {
                            ...
                            "volume_ratio": latest_block['volume'] / df['volume'].mean() if df['volume'].mean() > 0 else 0,
                            "distance_pct": (current_price - latest_block['price']) / latest_block['price'] * 100 if latest_block['price'] > 0 else 0
                        }
                    }
                    ...
                    return {
                        ...
                        "details": {
                            "block_price": latest_block['price'],
                            "volume_ratio": latest_block['volume'] / df['volume'].mean() if df['volume'].mean() > 0 else 0,
                            "distance_pct": (latest_block['price'] - current_price) / current_price * 100 if current_price > 0 else 0
                        }
                    }
    ...
    def break_of_structure(df):
        ...
        try:
            ...
            if current_price > recent_high:
                # Bullish break
                volume_sma = df['volume'].rolling(20).mean().iloc[-1] if len(df) >= 20 else df['volume'].mean()
                ...
                return {
                    ...
                    "details": {
                        ...
                        "break_distance": (current_price - recent_high) / recent_high * 100 if recent_high > 0 else 0,
                ...
                return {
                    ...
                    "details": {
                        ...
                        "break_distance": (recent_low - current_price) / current_price * 100 if current_price > 0 else 0,
                        "volume_confirmed": volume_confirmation
                    }
                }
                    ...
                    if current_price <= latest_fvg['price_level'] * 1.01:  # Within 1%
                        return {
                            ...
                            "details": {
                                ...
                                "gap_size_pct": latest_fvg['gap_size'] * 100 if 'gap_size' in latest_fvg else 0,
                                "distance_to_fvg": (current_price - latest_fvg['price_level']) / latest_fvg['price_level'] * 100 if latest_fvg['price_level'] > 0 else 0
                            }
                        }
                    ...
                    if current_price >= latest_fvg['price_level'] * 0.99:  # Within 1%
                        return {
                            ...
                            "details": {
                                "fvg_level": latest_fvg['price_level'],
                                "gap_size_pct": latest_fvg['gap_size'] * 100 if 'gap_size' in latest_fvg else 0,
                                "distance_to_fvg": (latest_fvg['price_level'] - current_price) / current_price * 100 if current_price > 0 else 0
                            }
                        }
    ...
    def ema_macd_combo(df):
        ...
        try:
            ...
            # Calculate EMAs
            ema_12 = AdvancedIndicators.calculate_ema(df['close'], 12).iloc[-1]
            ema_26 = AdvancedIndicators.calculate_ema(df['close'], 26).iloc[-1]
            ema_50 = AdvancedIndicators.calculate_ema(df['close'], 50).iloc[-1]
            
            # Calculate MACD
            macd_line, macd_signal, macd_hist = AdvancedIndicators.calculate_macd(df['close'])
            macd_val = macd_line.iloc[-1] if not macd_line.empty else 0
            macd_signal_val = macd_signal.iloc[-1] if not macd_signal.empty else 0
    ...
    def supertrend_bollinger_combo(df):
        ...
        try:
            ...
            current_supertrend = supertrend.iloc[-1] if not supertrend.empty else 0
            ...
            bb_upper_val = bb_upper.iloc[-1] if not bb_upper.empty else 0
            bb_middle_val = bb_middle.iloc[-1] if not bb_middle.empty else 0
            bb_lower_val = bb_lower.iloc[-1] if not bb_lower.empty else 0
...
class MomentumStrategies:
    ...
    def momentum_break_detection(df):
        ...
        try:
            if not is_valid_dataframe(df, 15):
                return {"signal": "NEUTRAL", "score": 0.0, "reason": "Momentum_Break", "type": "momentum"}
            
            # Calculate momentum indicators
            rsi_series = AdvancedIndicators.calculate_rsi(df['close'], 14)
            rsi = rsi_series.iloc[-1] if not rsi_series.empty else 50
            
            # Simple momentum calculation
            if len(df) >= 11:
                momentum = (df['close'].iloc[-1] - df['close'].iloc[-11]) / df['close'].iloc[-11] * 100
            else:
                momentum = 0
            ...
            # Price acceleration
            if len(df) >= 11:
                price_change_5 = (df['close'].iloc[-1] - df['close'].iloc[-6]) / df['close'].iloc[-6] if df['close'].iloc[-6] > 0 else 0
                price_change_10 = (df['close'].iloc[-1] - df['close'].iloc[-11]) / df['close'].iloc[-11] if df['close'].iloc[-11] > 0 else 0
    ...
    def volume_spike_analysis(df):
        ...
        try:
            ...
            volume_sma_20 = df['volume'].rolling(20).mean().iloc[-1] if len(df) >= 20 else df['volume'].mean()
            volume_sma_50 = df['volume'].rolling(50).mean().iloc[-1] if len(df) >= 50 else df['volume'].mean()
            current_volume = df['volume'].iloc[-1]
    ...
    def ema_golden_cross(df):
        ...
        try:
            ...
            # Calculate EMAs
            ema_8 = AdvancedIndicators.calculate_ema(df['close'], 8).iloc[-1]
            ema_21 = AdvancedIndicators.calculate_ema(df['close'], 21).iloc[-1]
            ema_50 = AdvancedIndicators.calculate_ema(df['close'], 50).iloc[-1]
            
            # Previous values for cross detection
            ema_8_series = AdvancedIndicators.calculate_ema(df['close'], 8)
            ema_21_series = AdvancedIndicators.calculate_ema(df['close'], 21)
            
            ema_8_prev = ema_8_series.iloc[-2] if len(ema_8_series) >= 2 else ema_8
            ema_21_prev = ema_21_series.iloc[-2] if len(ema_21_series) >= 2 else ema_21
...
class BreakoutStrategies:
    ...
    def resistance_support_breaks(df):
        ...
        try:
            if not is_valid_dataframe(df, 25):
            ...
            range_pct = consolidation_range / recent_low if recent_low > 0 else 0
    ...
    def bollinger_breakout_detection(df):
        ...
        try:
            ...
            current_price = df['close'].iloc[-1]
            upper_band = bb_upper.iloc[-1] if not bb_upper.empty else 0
            lower_band = bb_lower.iloc[-1] if not bb_lower.empty else 0
            middle_band = bb_middle.iloc[-1] if not bb_middle.empty else 0
...
class MeanReversionStrategies:
    ...
    def price_rejection_signals(df):
        ...
        try:
            if not is_valid_dataframe(df, 5):
...
class HiddenStrategies:
    ...
    def fibonacci_vortex_strategy(df):
        ...
        try:
            ...
            vi_plus_val = vi_plus.iloc[-1] if not vi_plus.empty else 1
            vi_minus_val = vi_minus.iloc[-1] if not vi_minus.empty else 1
            ...
            if nearest_level in ['0.618', '0.786'] and vortex_ratio > golden_ratio * 0.9:
                ...
                return {
                    ...
                    "details": {
                        ...
                        "golden_ratio_match": abs(vortex_ratio - golden_ratio) / golden_ratio * 100 if golden_ratio > 0 else 0
                    }
                }
                ...
                return {
                    ...
                    "details": {
                        "fib_level": nearest_level,
                        "vortex_ratio": vortex_ratio,
                        "golden_ratio_match": abs(vortex_ratio - (1/golden_ratio)) / (1/golden_ratio) * 100 if (1/golden_ratio) > 0 else 0
                    }
                }
    ...
    def quantum_entanglement_strategy(df):
        ...
        try:
            ...
            if len(prices) >= 10:
                price_uncertainty = np.std(prices[-10:]) / np.mean(prices[-10:]) if np.mean(prices[-10:]) > 0 else 0
                momentum_uncertainty = np.std(np.diff(prices[-10:])) / np.mean(np.abs(np.diff(prices[-10:]))) if np.mean(np.abs(np.diff(prices[-10:]))) > 0 else 0
    ...
    def dark_pool_institutional_strategy(df):
        ...
        try:
            ...
            # Strategy logic
            if dark_pool_data['is_dark_pool']:
                if dark_pool_data['imbalance'] > 0.3:
                    ...
                    return {
                        ...
                        "details": {
                            ...
                            "price_change": dark_pool_data['price_change']
                        }
                    }
                ...
                if nearest_gann['type'] == 'SUPPORT' and nearest_gann['strength'] > 0.7:
                    ...
                    return {
                        ...
                        "details": {
                            ...
                            "distance_to_gann": (current_price - nearest_gann['level']) / nearest_gann['level'] * 100 if nearest_gann['level'] > 0 else 0
                        }
                    }
                    ...
                    return {
                        ...
                        "details": {
                            "time_square": time_square,
                            "gann_strength": nearest_gann['strength'],
                            "distance_to_gann": (nearest_gann['level'] - current_price) / current_price * 100 if current_price > 0 else 0
                        }
                    }
        ...
        try:
            ...
            if wave_data['pattern'] == 'IMPULSE':
                ...
                if len(wave_data['waves']) >= 3:
                    ...
                    if current_wave == 5 and is_fib_valid:
                        ...
                        if current_price < target:
                            ...
                            return {
                                ...
                                "details": {
                                    ...
                                    "fib_relationship": fib_relationship,
                                    "target_price": target,
                                    "distance_to_target": (current_price - target) / target * 100 if target > 0 else 0
                                }
                            }
    ...
    def cosmic_movement_strategy(df, timestamp):
        ...
        try:
            ...
            current_price = df['close'].iloc[-1]
            if len(df) >= 2:
                price_change = (current_price - df['close'].iloc[-2]) / df['close'].iloc[-2] if df['close'].iloc[-2] > 0 else 0
            else:
                price_change = 0
            
            # Strategy logic
            if cosmic_data['is_positive']:
                # Positive cosmic alignment
                if price_change > 0:
                    return {
                        "signal": "BUY",
                        "score": 96.0,
...
class QuantumAnalysisEngine:
    ...
    def analyze_multi_timeframe(self, asset: str, tf_data: Dict[str, pd.DataFrame]) -> Dict:
        ...
        if len(buy_signals) >= len(sell_signals) and len(buy_signals) >= 3:
            avg_confidence = sum(s["score"] for s in buy_signals) / len(buy_signals) if buy_signals else 0
            return {
                "signal": "BUY",
            ...
            avg_confidence = sum(s["score"] for s in sell_signals) / len(sell_signals) if sell_signals else 0
            return {
                "signal": "SELL",
                "confidence": avg_confidence,
    ...
    def fetch_ohlcv(self, symbol: str, timeframe: str = "1h", limit: int = 200) -> Optional[pd.DataFrame]:
        ...
        if cache_key in self.cache:
            ...
            if (datetime.utcnow() - cache_time).total_seconds() < self.cache_timeout:
                if is_valid_dataframe(cached_df):
                    return cached_df.tail(min(limit, len(cached_df)))
...
class MarketFilters:
    ...
    def check_market_conditions(df: pd.DataFrame, asset_config: Dict) -> Tuple[bool, str]:
        ...
        try:
            ...
            if len(returns) < 10:
                return False, "Not enough data for volatility check"
            ...
            atr = AdvancedIndicators.calculate_atr(df['high'], df['low'], df['close'], 14)
            if atr.empty:
                return False, "Cannot calculate ATR"
                
            atr_val = atr.iloc[-1]
            atr_ratio = atr_val / df['close'].iloc[-1] if df['close'].iloc[-1] > 0 else 0
            
            if atr_ratio < asset_config.get("choppy_threshold", 0.35):
                # Additional choppiness check
                if len(df) >= 20:
                    recent_range = (df['high'].iloc[-20:].max() - df['low'].iloc[-20:].min()) / df['close'].iloc[-20:].mean() if df['close'].iloc[-20:].mean() > 0 else 0
            ...
            ema_12 = AdvancedIndicators.calculate_ema(df['close'], 12).iloc[-1]
            ema_26 = AdvancedIndicators.calculate_ema(df['close'], 26).iloc[-1]
            ema_diff = abs(ema_12 - ema_26) / df['close'].iloc[-1] if df['close'].iloc[-1] > 0 else 0
...
class QuantumTelegramBot:
    ...
    def _format_signal(self, signal_data: Dict) -> str:
        ...
        strategies_text = "\n".join([
            f"  • {s.get('reason', 'Unknown')} ({s.get('score', 0):.0f}%)" 
        ...
        return f"""{mode_emoji} <b>QUANTUM SIGNAL - {signal_data.get('mode', 'quantum').upper()}</b>

🎯 <b>Asset:</b> {signal_data.get('asset', 'N/A')}
📈 <b>Direction:</b> {signal_data.get('direction', 'NEUTRAL')}
⏱️ <b>Expiry:</b> {signal_data.get('expiry', 0)}m
🎯 <b>Confidence:</b> {signal_data.get('confidence', 0):.1f}%
...
#{signal_data.get('asset', 'N/A')} #TradingSignal"""
    ...
    def _generate_signals(self, asset: str, analysis_result: Dict, aligned_count: int):
        ...
        for expiry in EXPIRIES:
            ...
            signal_data = {
                ...
                "timeframe_alignment": aligned_count / 3 if aligned_count > 0 else 0,
