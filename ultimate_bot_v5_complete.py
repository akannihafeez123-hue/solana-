...
FIXED: All DataFrame comparison errors
"""
...
TELEGRAM_SEND_CONFIRMATION_ONLY = os.getenv("TELEGRAM_SEND_CONFIRMATION_ONLY", "True") == "True"
TELEGRAM_MIN_CONFIDENCE = int(os.getenv("TELEGRAM_MIN_CONFIDENCE", "90"))
TELEGRAM_COOLDOWN_MINUTES = int(os.getenv("TELEGRAM_COOLDOWN_MINUTES", "5"))
...
LAST_SIGNAL_TIME = {}
LAST_TELEGRAM_MESSAGE_TIME = None
...
class AdvancedIndicators:
    ...
    def calculate_supertrend(df, period=10, multiplier=3):
        ...
        try:
            ...
            for i in range(1, len(df)):
                if float(df['close'].iloc[i-1]) <= float(supertrend.iloc[i-1]):
                    if float(df['close'].iloc[i]) > float(upper_band.iloc[i]):
                        direction.iloc[i] = 1
                        supertrend.iloc[i] = lower_band.iloc[i]
                    else:
                        direction.iloc[i] = -1
                        supertrend.iloc[i] = min(float(upper_band.iloc[i]), float(supertrend.iloc[i-1]))
                else:
                    if float(df['close'].iloc[i]) < float(lower_band.iloc[i]):
                        direction.iloc[i] = -1
                        supertrend.iloc[i] = upper_band.iloc[i]
                    else:
                        direction.iloc[i] = 1
                        supertrend.iloc[i] = max(float(lower_band.iloc[i]), float(supertrend.iloc[i-1]))
    ...
    def calculate_order_blocks(df, lookback=20):
        ...
        try:
            ...
            for i in range(lookback, len(df)-lookback):
                ...
                volume_mean = float(df['volume'].iloc[i-lookback:i].mean())
                if volume_mean == 0:
                    continue
                    
                volume_ratio = float(df['volume'].iloc[i]) / volume_mean
                range_ratio = (float(df['high'].iloc[i]) - float(df['low'].iloc[i])) / float(df['close'].iloc[i])
                
                if volume_ratio > 2.5 and range_ratio < 0.01:
                    # Check if this is followed by a significant move
                    future_return = (float(df['close'].iloc[i+lookback]) - float(df['close'].iloc[i])) / float(df['close'].iloc[i])
    ...
    def detect_dark_pool_activity(df, threshold=3):
        ...
        try:
            ...
            price_change = abs(float(df['close'].iloc[-1]) - float(df['close'].iloc[-2])) / float(df['close'].iloc[-2])
            
            # Calculate order imbalance - FIXED: Use boolean mask correctly
            buy_mask = df['close'] > df['open']
            sell_mask = df['close'] < df['open']
            
            # Convert to boolean arrays
            buy_mask_array = buy_mask.values
            sell_mask_array = sell_mask.values
            
            # Get indices where conditions are True
            buy_indices = np.where(buy_mask_array)[0]
            sell_indices = np.where(sell_mask_array)[0]
            
            # Sum volumes using indices
            buy_volume = float(df['volume'].iloc[buy_indices].sum()) if len(buy_indices) > 0 else 0
            sell_volume = float(df['volume'].iloc[sell_indices].sum()) if len(sell_indices) > 0 else 0
...
class QuantumStrategies:
    ...
    def break_of_structure(df):
        ...
        try:
            ...
            if current_price > recent_high:
                ...
                # FIX: Convert to scalar before comparison
                current_volume = float(df['volume'].iloc[-1])
                volume_confirmation = current_volume > volume_sma * 1.5
    ...
    def ema_macd_combo(df):
        ...
        try:
            ...
            # Volume confirmation
            volume_sma = float(df['volume'].rolling(20).mean().iloc[-1])
            volume_ratio = float(df['volume'].iloc[-1]) / volume_sma if volume_sma > 0 else 1
...
class MomentumStrategies:
    ...
    def momentum_break_detection(df):
        ...
        try:
            ...
            # Calculate momentum
            current_close = float(df['close'].iloc[-1])
            close_minus_5 = float(df['close'].iloc[-5]) if len(df) >= 6 else current_close
            momentum = (current_close - close_minus_5) / close_minus_5
            ...
            # Price acceleration
            if len(df) >= 15:
                price_change_5 = (current_close - close_minus_5) / close_minus_5
                close_minus_10 = float(df['close'].iloc[-10]) if len(df) >= 11 else current_close
                price_change_10 = (current_close - close_minus_10) / close_minus_10
...
class HiddenStrategies:
    ...
    def quantum_entanglement_strategy(df):
        ...
        try:
            ...
            price_mean = np.mean(recent_prices) if len(recent_prices) > 0 else 0
            price_uncertainty = np.std(recent_prices) / price_mean if price_mean > 0 else 0
            
            momentum = np.diff(recent_prices)
            momentum_mean = np.mean(np.abs(momentum)) if len(momentum) > 0 else 0
            momentum_uncertainty = np.std(momentum) / momentum_mean if momentum_mean > 0 else 0
            
            uncertainty_product = price_uncertainty * momentum_uncertainty
            
            # Quantum probability wave
            yf_sum = np.sum(np.abs(yf)) if np.sum(np.abs(yf)) > 0 else 1
            probability_wave = np.abs(yf[dominant_idx]) / yf_sum
                    ...
                    if current_price > prev_price:
                        ...
                        return {
                            ...
                            "details": {
                                "resonance_match": True,
                                "dominant_frequency": dominant_freq,
                                "probability_wave": float(probability_momentum),
                                "uncertainty_product": float(uncertainty_product)
                            }
                        }
...
class MarketFilters:
    ...
    def check_market_conditions(df: pd.DataFrame, asset_config: Dict) -> Tuple[bool, str]:
        ...
        try:
            ...
            current_close = float(df['close'].iloc[-1])
            atr_ratio = float(atr) / current_close if current_close > 0 else 0
            
            if atr_ratio < asset_config.get("choppy_threshold", 0.35):
                # Additional choppiness check
                recent_high = float(df['high'].iloc[-20:].max())
                recent_low = float(df['low'].iloc[-20:].min())
                recent_mean = float(df['close'].iloc[-20:].mean())
...
class QuantumTelegramBot:
    """Enhanced Telegram bot with confirmation system only"""
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if data.get("ok"):
                    ...
                    # Send startup confirmation only
                    self.send_confirmation("STARTUP", "Quantum Trading Bot V6.0 initialized successfully!")
                    return True
        except Exception as e:
    ...
    def should_send_message(self) -> bool:
        """Check if we should send a message (cooldown)"""
        if self.last_message_time is None:
            return True
        
        elapsed = (datetime.utcnow() - self.last_message_time).total_seconds() / 60
        return elapsed >= TELEGRAM_COOLDOWN_MINUTES
    ...
    def send_confirmation(self, event_type: str, message: str):
        ...
        # Check cooldown
        if not self.should_send_message() and event_type not in ["STARTUP", "ERROR"]:
            logger.debug(f"⏳ Telegram cooldown active, skipping {event_type}")
            return False
        ...
        try:
            # Only send concise confirmation messages
            if TELEGRAM_SEND_CONFIRMATION_ONLY and event_type != "SIGNAL":
                text = f"{emoji} <b>QUANTUM BOT - {event_type}</b>\n\n{message}\n\n🕐 {datetime.utcnow().strftime('%H:%M:%S')} UTC"
            elif event_type == "SIGNAL":
                # For signals, check if we should send based on confidence
                if "Confidence:" in message:
                    try:
                        # Extract confidence from message
                        confidence_str = message.split("Confidence:")[1].split("%")[0].strip()
                        confidence = float(confidence_str)
                        if confidence < TELEGRAM_MIN_CONFIDENCE:
                            logger.info(f"📊 Signal below threshold ({confidence}% < {TELEGRAM_MIN_CONFIDENCE}%), not sending")
                            return False
                    except:
                        pass
                text = message
            else:
                return False  # Don't send other types if confirmation only
            ...
            if response.status_code == 200:
                ...
                logger.info(f"📱 Telegram {event_type} sent")
                return True
            else:
    ...
    def send_signal(self, signal_data: Dict):
        """Send trading signal only if high confidence"""
        if not self.initialized:
            return False
        
        # Check signal confidence
        if signal_data.get('confidence', 0) < TELEGRAM_MIN_CONFIDENCE:
            logger.info(f"📊 Signal below threshold ({signal_data.get('confidence', 0)}% < {TELEGRAM_MIN_CONFIDENCE}%), not sending")
            return False
        ...
        try:
            ...
            if response.status_code == 200:
                self.last_message_time = datetime.utcnow()
                logger.info(f"📱 High-confidence signal sent for {signal_data['asset']}")
                return True
            else:
                logger.error(f"❌ Telegram signal send failed: {response.text}")
                return False
        except Exception as e:
    ...
    def _format_signal(self, signal_data: Dict) -> str:
        ...
        # Simplified format for confirmation-only mode
        if TELEGRAM_SEND_CONFIRMATION_ONLY:
            return f"""{mode_emoji} <b>QUANTUM SIGNAL CONFIRMED</b>
...
🕐 {datetime.utcnow().strftime('%H:%M:%S')} UTC
#{signal_data['asset']} #{signal_data['direction']}"""
        
        # Original format (if not in confirmation-only mode)
        strategies_text = "\n".join([
...
class QuantumTradingEngine:
    ...
    def __init__(self):
        ...
        if TELEGRAM_TOKEN and CHAT_ID:
            ...
            if self.telegram_bot.start():
                logger.info("✅ Telegram bot initialized with confirmation-only mode")
    
    def scan_assets(self):
        """Main scanning loop with minimal notifications"""
        ...
        for asset in ASSETS:
            try:
                ...
                # Only send error confirmations
                if self.telegram_bot and "critical" in error_msg.lower():
                    self.telegram_bot.send_confirmation("ERROR", error_msg[:100])
        
        logger.info(f"✅ Scan completed for {len(ASSETS)} assets")
    ...
    def _analyze_asset(self, asset: str):
        ...
        try:
            ...
            if not is_aligned:
                logger.debug(f"  ✗ Timeframe misalignment: {aligned_count}/{len(tf_data)}")
                return
    ...
    def _generate_signals(self, asset: str, analysis_result: Dict, aligned_count: int, total_timeframes: int):
        ...
        if analysis_result["confidence"] < TELEGRAM_MIN_CONFIDENCE:
            logger.debug(f"  📊 Signal below Telegram threshold: {analysis_result['confidence']:.1f}%")
            return
        ...
        for expiry in EXPIRIES:
            ...
            # Send Telegram alert only for high confidence
    ...
    def change_mode(self, new_mode: str):
        ...
        try:
            ...
            if self.telegram_bot:
                self.telegram_bot.send_confirmation("WARNING", f"Mode changed to {self.current_mode.value.upper()}")
...
def main():
    ...
    ║          WITH MINIMAL TELEGRAM CONFIRMATIONS             ║
    ╚══════════════════════════════════════════════════════════╝
    
    ✅ Fixed Issues:
      • ✅ ALL DataFrame comparison errors resolved
      • ✅ Telegram confirmation-only mode
