...
ULTIMATE ALL-IN-ONE VERSION - Optimized for Replit
"""
...
if not TELEGRAM_TOKEN or not CHAT_ID:
    print(f"   ⚠️  Set TELEGRAM_TOKEN and CHAT_ID in .env file or Secrets")
    print(f"   Example .env file:")
    print(f"   TELEGRAM_TOKEN=1234567890:ABCdefGHIjklMNOpqrsTUVwxyz")
    print(f"   CHAT_ID=987654321")
...
class LegendaryTelegramBot:
    ...
    def start(self):
        """Initialize Telegram bot with detailed feedback"""
        if not TELEGRAM_AVAILABLE:
            logger.warning("⚠️ Telegram modules not installed")
            logger.warning("Install with: pip install python-telegram-bot python-dotenv")
            return False
            
        if not self.token:
            logger.warning("⚠️ Telegram token not configured")
            logger.warning("Set TELEGRAM_TOKEN in .env file or Secrets")
            return False
            
        if not self.chat_id:
            logger.warning("⚠️ Chat ID not configured")
            logger.warning("Set CHAT_ID in .env file or Secrets")
            return False
        ...
        try:
            ...
            if not self.token or ":" not in self.token:
                ...
                logger.error("Token should be like: 1234567890:ABCdefGHIjklMNOpqrsTUVwxyz")
                return False
            ...
            try:
                ...
                logger.error("Chat ID should be numeric (like: 987654321)")
                return False
            
            self.bot = Bot(token=self.token, request_timeout=30)
            ...
            if "invalid token" in error_str or "unauthorized" in error_str:
                logger.error("   Reason: Invalid bot token")
                logger.error("   Check your TELEGRAM_TOKEN in .env file")
            elif "timed out" in error_str:
                logger.error("   Reason: Connection timeout")
                logger.error("   Check your internet connection")
            elif "chat not found" in error_str:
                logger.error("   Reason: Chat ID not found")
                logger.error("   Make sure you've sent /start to your bot")
            else:
                logger.error("   Check your token and internet connection")
    ...
    def send_startup_message(self):
        ...
        try:
            message = f"""🤖 <b>ULTIMATE TRADING BOT STARTED</b>

✅ Bot initialized successfully
⏰ Time: {datetime.utcnow().strftime('%Y-%m-%d %H:%M:%S')} UTC
📈 Scanning: {len(ASSETS)} assets
🎯 Confidence: {LEGENDARY_GATE}% threshold
...
Ready to scan for legendary signals! 🚀"""
    ...
    async def send_message_async(self, text: str):
        ...
        if not self.initialized or not self.bot:
            logger.warning("Cannot send: Telegram bot not initialized")
            return False
        
        try:
            logger.info(f"📤 Sending Telegram message ({len(text)} chars)...")
            ...
            logger.info(f"✅ Telegram message #{self.message_count} sent successfully!")
            ...
            if "chat not found" in error_str:
                logger.error("   Fix: Send /start to your bot first")
            elif "unauthorized" in error_str:
                logger.error("   Fix: Check your bot token")
            elif "flood" in error_str:
                logger.error("   Fix: Reduce message frequency")
            elif "bad request" in error_str:
                logger.error("   Fix: Check message formatting")
    ...
    def send_message(self, text: str):
        ...
        try:
            ...
            # Send the message
            success = loop.run_until_complete(self.send_message_async(text))
    ...
    def send_signal(self, alert_data: Dict):
        ...
        if not self.initialized:
            logger.warning("Cannot send signal: Telegram not initialized")
            return False
        ...
        try:
            ...
            if success:
                ...
                logger.warning(f"⚠️ Failed to send signal to Telegram")
                return False
...
def test_telegram_connection():
    ...
    if not TELEGRAM_TOKEN:
        ...
        print("   Add to .env file: TELEGRAM_TOKEN=your_token_here")
        return False
    
    if not CHAT_ID:
        print("❌ CHAT_ID not set")
        print("   Add to .env file: CHAT_ID=your_chat_id_here")
        return False
    
    telegram = LegendaryTelegramBot(TELEGRAM_TOKEN, CHAT_ID)
    success = telegram.start()
    
    if success and telegram.initialized:
        print("✅ Telegram bot initialized successfully!")
        
        # Send a test message
        test_msg = f"""🧪 <b>BOT CONNECTION TEST</b>

✅ Telegram connection successful!
🤖 Bot is ready to send signals
⏰ Test time: {datetime.utcnow().strftime('%Y-%m-%d %H:%M:%S')} UTC

You will receive trading signals here when detected! 🚀"""
        
        print("📤 Sending test message...")
        message_sent = telegram.send_message(test_msg)
...
if FLASK_AVAILABLE:
    ...
    def health_check():
        ...
        return jsonify({
            ...
            "telegram_initialized": bool(telegram_bot.initialized if 'telegram_bot' in globals() else False)
        }), 200
...
def main_scan_loop():
    ...
    global ASSETS, telegram_bot
    
    logger.info("🏆 Initializing ULTIMATE UNSTOPPABLE BOT")
    ...
    logger.info(f"🌐 Flask Web Interface: {'enabled' if FLASK_AVAILABLE else 'disabled'}")
    logger.info(f"🎯 Confidence Threshold: {LEGENDARY_GATE}%")

    provider = SuperReliableDataProvider()
    telegram_bot = LegendaryTelegramBot(TELEGRAM_TOKEN, CHAT_ID)
    ...
    if TELEGRAM_TOKEN and CHAT_ID:
        ...
        if telegram_ready:
            ...
            logger.warning("⚠️ Telegram bot failed to initialize")
    else:
    ...
    while True:
        try:
            ...
            if 'telegram_bot' in globals() and telegram_bot.initialized:
...
def main():
    ...
    if TELEGRAM_TOKEN and CHAT_ID:
        ...
        print(f"\n💡 TIP: Check your Telegram app for a test message")
        print(f"   If no message appears, check your .env file or Secrets")
    else:
        ...
        print(f"   3. Add TELEGRAM_TOKEN and CHAT_ID to .env file or Secrets")
