...
FIXED: DataFrame comparison errors, optimized Telegram messages
"""
...
class AdvancedIndicators:
    ...
    def detect_dark_pool_activity(df, threshold=3):
        ...
        try:
            ...
            # FIX: Convert Series to boolean array before using in loc
            buy_mask_bool = buy_mask.values if hasattr(buy_mask, 'values') else buy_mask
            sell_mask_bool = sell_mask.values if hasattr(sell_mask, 'values') else sell_mask
            
            buy_volume = float(df.loc[buy_mask_bool, 'volume'].sum())
            sell_volume = float(df.loc[sell_mask_bool, 'volume'].sum())
...
class MomentumStrategies:
    ...
    def momentum_break_detection(df):
        ...
        try:
            ...
            # Calculate momentum
            momentum = (float(df['close'].iloc[-1]) - float(df['close'].iloc[-5])) / float(df['close'].iloc[-5]) if len(df) >= 6 else 0
            ...
            # Price acceleration
            if len(df) >= 15:
                price_change_5 = (float(df['close'].iloc[-1]) - float(df['close'].iloc[-5])) / float(df['close'].iloc[-5])
                price_change_10 = (float(df['close'].iloc[-1]) - float(df['close'].iloc[-10])) / float(df['close'].iloc[-10])
    ...
    def volume_spike_analysis(df):
        ...
        try:
            ...
            volume_change = (current_volume - float(df['volume'].iloc[-2])) / float(df['volume'].iloc[-2])
...
class MarketFilters:
    ...
    def check_market_conditions(df: pd.DataFrame, asset_config: Dict) -> Tuple[bool, str]:
        ...
        try:
            ...
            atr_ratio = float(atr) / float(df['close'].iloc[-1])
            
            if atr_ratio < asset_config.get("choppy_threshold", 0.35):
                # Additional choppiness check
                recent_range = (float(df['high'].iloc[-20:].max()) - float(df['low'].iloc[-20:].min())) / float(df['close'].iloc[-20:].mean())
